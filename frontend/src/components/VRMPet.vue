...
const { currentVRMModel, availableVRMModels, vrmModelURL, loadVRMModels } =
  useVRMModel();
...
function updateMouthAnim(dt) {
  ...
  if (petState.value === "speaking") {
    ...
    if (cur > 0.001) {
      try {
        vrm.expressionManager.setValue("aa", Math.max(0, cur - dt * 2));
      } catch (_) {}
    }
  }
}
...
function updateEmotionBlend(dt) {
  ...
  const allKeys = new Set([
    ...Object.keys(targetEmotionWeights),
    ...Object.keys(currentEmotionWeights),
  ]);
...
function scheduleBlink() {
  blinkTimer = setTimeout(
    async () => {
      if (!mounted || !vrm?.expressionManager) {
        scheduleBlink();
        return;
      }
      try {
...
async function switchToNextVRMModel() {
  ...
  try {
    ...
    if (cfg) {
      cfg.VRMModel = next.name;
      await SaveConfig(cfg);
    }
  } catch (e) {
    console.warn("switchToNextVRMModel:", e);
  }
}
...
function onMouseMove(e) {
  ...
  if (vrm)
    vrm.scene.rotation.y =
      vrmBaseRotationY + Math.max(-0.35, Math.min(0.35, dx / 200));
}
  ...
  if (isDragging) {
    ...
    try {
      await SaveBallPosition(
        Math.round(pos.value.x),
        Math.round(pos.value.y),
        sw.value,
        sh.value,
      );
    } catch (err) {
      console.error("SaveBallPosition:", err);
    }
...
watch(vrmModelURL, async (url) => {
  ...
  try {
    ...
    console.error("VRM reload failed:", err);
  }
});
...
onMounted(async () => {
  ...
  try {
    ...
    pos.value = {
      x: window.innerWidth - petSize.value - 40,
      y: window.innerHeight - petSize.value - 40,
    };
  }
