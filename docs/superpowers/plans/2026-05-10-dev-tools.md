# Developer Utility Tools Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add 12 stateless developer utility tools to Aiko's built-in tool registry covering JSON, encoding, hashing, UUID, timestamp, regex, number bases, unit conversion, color conversion, and exchange rates.

**Architecture:** All 12 tools live in a single new file `internal/tools/dev_tools.go`. They are registered by appending to `All()` in `internal/tools/registry.go`. `AllPermissionDeclarations()` picks them up automatically. Tests go in `internal/tools/dev_tools_test.go`.

**Tech Stack:** Go stdlib (`encoding/json`, `encoding/base64`, `net/url`, `html`, `crypto/*`, `regexp`, `strconv`, `math`), `gopkg.in/yaml.v3` (already indirect dep), `github.com/google/uuid` (already dep), `net/http` for exchange rate.
...
|--------|------|----------------|
| Create | `internal/tools/dev_tools.go` | All 12 tool structs + InvokableRun implementations |
| Create | `internal/tools/dev_tools_test.go` | Unit tests for all 12 tools |
| Modify | `internal/tools/registry.go` | Append 12 tools to `All()` |

---

## Task 1: JSON Tools (format_json, json_to_struct, yaml_json_convert)

**Files:**
- Create: `internal/tools/dev_tools.go`
- Create: `internal/tools/dev_tools_test.go`
- Modify: `internal/tools/registry.go`

- [ ] **Step 1: Write failing tests for the 3 JSON tools**
...
func TestJSONToStruct(t *testing.T) {
	...
	t.Run("go", func(t *testing.T) {
		out, err := tool.InvokableRun(nil, `{"json_string":"`+input+`","language":"go","type_name":"User"}`)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("typescript", func(t *testing.T) {
		out, err := tool.InvokableRun(nil, `{"json_string":"`+input+`","language":"typescript","type_name":"User"}`)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("python", func(t *testing.T) {
		out, err := tool.InvokableRun(nil, `{"json_string":"`+input+`","language":"python","type_name":"User"}`)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("rust", func(t *testing.T) {
		out, err := tool.InvokableRun(nil, `{"json_string":"`+input+`","language":"rust","type_name":"User"}`)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("nested_object", func(t *testing.T) {
		nested := `{"user":{"name":"Bob","age":25},"count":3}`
		out, err := tool.InvokableRun(nil, `{"json_string":"`+nested+`","language":"go","type_name":"Root"}`)
		if err != nil {
			t.Fatal(err)
		}
...
```bash
cd /Users/xutiancheng/code/self/Aiko && go test ./internal/tools/... -run "TestFormatJSON|TestJSONToStruct|TestYAMLJSONConvert" -v 2>&1 | head -30
```

Expected: compile error — types `FormatJSONTool`, `JSONToStructTool`, `YAMLJSONConvertTool` undefined.

- [ ] **Step 3: Create dev_tools.go with the 3 JSON tools**
...
func (t *FormatJSONTool) InvokableRun(_ context.Context, input string, _ ...tool.Option) (string, error) {
	...
	if err := json.Unmarshal([]byte(jsonStr), &raw); err != nil {
		if action == "validate" {
			return fmt.Sprintf("无效 JSON — 错误: %s", err.Error()), nil
		}
...
func (t *YAMLJSONConvertTool) InvokableRun(_ context.Context, input string, _ ...tool.Option) (string, error) {
	...
	case "yaml_to_json":
		...
		// yaml.v3 returns map[string]any compatible with sonic
		var b []byte
		var err error
		if pretty {
...
func (t *GetExchangeRateTool) InvokableRun(_ context.Context, input string, _ ...tool.Option) (string, error) {
	...
	// Parse targets (accept JSON array string or comma-separated).
	var targets []string
	...
	for _, t := range targets {
		rate, ok := result.Rates[strings.ToUpper(t)]
		if !ok {
			lines = append(lines, fmt.Sprintf("%s: 不支持", t))
			continue
		}
		converted := amount * rate
		lines = append(lines, fmt.Sprintf("%s: 1 %s = %.6f %s → %g %s = %.2f %s",
			t, base, rate, t, amount, base, converted, t))
	}
	return strings.Join(lines, "\n"), nil
}
...
// Ensure unused imports are referenced (bytes, io referenced in exchange rate tool via io.LimitReader and bytes.Buffer placeholder).
...
```bash
cd /Users/xutiancheng/code/self/Aiko && go test ./internal/tools/... -run "TestFormatJSON|TestJSONToStruct|TestYAMLJSONConvert" -v 2>&1
```

Expected: all 3 test functions PASS.

- [ ] **Step 5: Register the 12 tools in registry.go**

In `internal/tools/registry.go`, inside `All()`, append after `&CreateCalendarEventTool{}`:
...
git add internal/tools/dev_tools.go internal/tools/dev_tools_test.go internal/tools/registry.go
git commit -m "feat(tools): add 12 developer utility tools

JSON format/minify/validate, JSON→struct (Go/TS/Python/Rust),
YAML↔JSON, base64/URL/HTML encode-decode, hash (MD5/SHA*),
UUID v4, timestamp convert, regex test, number base convert,
unit convert, exchange rate, color format convert."
```

---

## Task 2: Remaining Tool Tests

**Files:**
- Modify: `internal/tools/dev_tools_test.go`

- [ ] **Step 1: Add tests for tools 4–12**

Append to `internal/tools/dev_tools_test.go`:
...
```bash
cd /Users/xutiancheng/code/self/Aiko && go test ./internal/tools/... -v 2>&1
```

Expected: all tests PASS (exchange rate test not included — requires live network).

- [ ] **Step 3: Fix any test failures**

If any test fails, diagnose and fix the implementation in `dev_tools.go`, then re-run until all pass.

- [ ] **Step 4: Commit**

```bash
cd /Users/xutiancheng/code/self/Aiko
git add internal/tools/dev_tools_test.go
git commit -m "test(tools): add tests for all 12 developer utility tools"
```

---

## Task 3: Remove unused import and final verification

**Files:**
- Modify: `internal/tools/dev_tools.go`

- [ ] **Step 1: Remove the `bytes` placeholder import**

In `dev_tools.go`, remove the line:
```go
var _ = bytes.NewBuffer
```
And remove `"bytes"` from the import block (it was added only as a placeholder; `io` is still used via `io.LimitReader` in `get_exchange_rate`).

- [ ] **Step 2: Final build + full test run**

```bash
cd /Users/xutiancheng/code/self/Aiko && go build ./... && go test ./internal/tools/... -v 2>&1
```
...
```bash
cd /Users/xutiancheng/code/self/Aiko
git add internal/tools/dev_tools.go
git commit -m "chore(tools): clean up dev_tools.go imports"
```
...
**Spec coverage check:**
- ✅ `format_json` — pretty/minify/validate, indent param
- ✅ `json_to_struct` — Go/TS/Python/Rust, nested objects, type_name param, type inference rules
- ✅ `yaml_json_convert` — both directions, pretty param
- ✅ `encode_decode` — base64/base64url/url/html encode+decode
- ✅ `hash_text` — md5/sha1/sha256/sha512, hex+base64 output
- ✅ `generate_uuid` — count 1-100, standard/no_dash/upper formats
- ✅ `convert_timestamp` — both directions, multi-timezone output on unix→datetime
- ✅ `regex_test` — match/no-match, all substrings, capture groups
- ✅ `number_base_convert` — 2/8/10/16, prefix on output
- ✅ `convert_units` — all 7 categories including temperature special case
- ✅ `get_exchange_rate` — open.er-api.com, targets param, amount param, PermProtected
- ✅ `convert_color` — #RRGGBB/#RGB/rgb()/hsl() input, hex/rgb/hsl/all output
- ✅ All 12 tools registered in `All()` in `registry.go`
- ✅ `AllPermissionDeclarations()` picks them up automatically via `All()`

**Type consistency:** All struct names in Task 1 code match the names in the registry step and the test file. `convertUnits` helper function is called only from `ConvertUnitsTool.InvokableRun`. `sortedKeys` and helper functions are package-level and shared within `dev_tools.go`.

**Placeholder scan:** No TBD/TODO. All test cases include actual inputs and expected outputs. All implementation code is complete.
